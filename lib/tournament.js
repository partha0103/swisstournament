...
function registerPlayer(user_id, tournament_id, name, cb){
    ...
    console.log(tournament_id, "tour");
    console.log(name, "name");
    var connection = get_connection();
    var stmt = "select count(distinct name) as count from players where user_id = ? and name = ?"
...
function winner(tournament_id, cb){
    var stmt = "select winner from tournaments where id = ?";
    var connection = get_connection();
    connection.query(stmt,tournament_id,function(error, results){
        connection.end();
        if(error){
...
function addExistingPlayers(user_id, tournament_id, cb){
    var stmt = `select distinct name from players where user_id=? and tournament_id not in (?) and
                             name not in(select name from players where tournament_id=?)`
    var connection = get_connection();
