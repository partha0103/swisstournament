...
function playersInTour(tournament_id, cb){
    var stmt = "select name from players where tournament_id= ?";
    var connection = get_connection();
...
function currentStandings(tournament_id,cb){
    var connection = get_connection();
    var stmt = `select p.id, p.name, m.matches as ma, w.wins as wins from players as p join matchs_count as m on p.id = m.id join win_count as w on   p.id = w.id where p.tournament_id = ? order by wins desc;`
    connection.query(stmt,tournament_id,function(error,results) {
        connection.end();
        if(error){
            throw error;
