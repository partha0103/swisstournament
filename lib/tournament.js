...
function playersInTour(tournament_id, cb){
    var stmt = "select name from player where tournament_id= ?";
    var connection = get_connection();
...
function playerDetails(user_id,tournament_id, cb){
    playerInTour(tournament_id, function(t_player){
        var connection = get_connection();
    ...
    if(isStarted(tournament_id, function(flag){
        if(flag){
            ...
            connection.query(stmt,[user_id,tournament_id,name],function(error, results){
                ...
                players(user_id,tournament_id, function(u_players, t_player){
                    cb(u_players, t_player);
                });
            });
        }
...
function deletePlayers(cb){
    var connection = get_connection();
    var stmt = "delete from player where tournament_id = ?";
    connection.query(stmt,tournament_id,function(error,result) {
        connection.end();
        if(error){
            throw error;
...
function currentStandings(tournament_id,cb){
    var connection = get_connection();
    var stmt = `select p.player_id, p.name, m.matches as ma, w.wins as wins from player as p join matchs_count as m on p.player_id = m.player_id join win_count as w on   p.player_id = w.player_id where p.tournament_id = ? order by wins desc;`
    connection.query(stmt,tournament_id,function(error,results) {
        connection.end();
        if(error){
            throw error;
