...
function play(tournament_id,winner,round, cb){
    swissPairings(tournament_id, function(matches){
        countPlayers(tournament_id, function(count){
            var max_round = Math.log2(count)-1;
            ...
            if(round == max_round){
                ...
                matches.forEach(function(match, index, matches){
                    ...
                    w_list.push(c_match);
                    var stmt = "insert into matches(tournament_id, player1_id, player2_id, winner_id,round) values(?,?,?,?,?)"
                    var connection = get_connection();
                    connection.query(stmt, [tournament_id,player1, player2, winner_id, round], function(error, result){
                        connection.end();
                        if(error)
                            throw error;
                        else if(index == matches.length-1){
