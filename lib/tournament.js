...
function tournamentStatus(tournament_id, cb){
    ...
    buildMatches(tournament_id,function(matches){
        currentStandings(tournament_id,function(standings){
            ...
            while(standings.length > 0){
                ...
                var player1 = standings.splice(0,1)[0].name;
                for(let i = 0; i < standings.length ; i++){
                    var player2 = standings[i].name;
                    var match1 = {
                        player1: player1,
                        player2: player2
                    };
        ...
        countPlayers(tournament_id, function(count){
            ...
            if(round == max_round){
                ...
                matches.forEach(function(match, index, matches){
                    ...
                    w_list.push(c_match);
                    var stmt = "insert into matches(tournament_id, player1_name, player2_name, winner_name,round, r_status) values(?,?,?,?,?,?)"
                    var connection = get_connection();
                    connection.query(stmt, [tournament_id,player1, player2, m_winner, round, status], function(error, result){
                        connection.end();
                        if(error)
                            throw error;
                        else if(index == matches.length-1){
...
function updateTstatus(tournament_id,status,cb){
    var stmt = "update tournaments  set status = ?  where  tournament_id = ?";
    var connection = get_connection();
...
function getTstatus(tournament_id, cb){
    var stmt = "select status from from tournaments where tournament_id = ?";
    var connection = get_connection();
    connection.query(stmt,[status,tournament_id],function(error, results){
        connection.end();
        if(error){
